...
void logEvent(String message) {
  ...
  if (!file) {
    Serial.println("[FS_ERR] Не вдалося відкрити файл логів!");
    return;
  }
  
  String logString = "[" + String(millis() / 1000) + "s] " + message + "\n";
  file.print(logString);
  file.close();
  
  Serial.print("[FS_LOG] Записано: " + logString);
}
...
void loop() {
  ...
  if (triggerRemoteUnlock) {
    ...
    logEvent("ACCESS_GRANTED: REMOTE_ADMIN"); 
  ...
  if (currentButtonState != lastButtonState) {
    if ((millis() - lastDebounceTime) > debounceDelay) {
      ...
      if (currentButtonState == HIGH) {
        ...
        logEvent("BUTTON_PRESSED: CALLING_ADMIN"); 
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      if (access) {
        logEvent("ACCESS_GRANTED: UID_" + uidServer); 
        executeUnlockAnimation("> ACCESS GRANTED", msg);
      } else {
        logEvent("ACCESS_DENIED: UID_" + uidServer); 
